...
def parse_args():
    parser = argparse.ArgumentParser(description="Validate Stage1 IAAI auxiliary gyro model.")
...
def main():
    ...
    run_dir = create_run_dir(args.output_root, "stage1_iaai_validation")
    visual_dir = run_dir / "visuals"

    split = args.split or cfg.get("validation", {}).get("split") or "val"
    val_cfg = cfg.get("validation", {})
    dataset = build_stage1_iaai_dataset(cfg, split=split)
    loader = DataLoader(
        dataset,
    ...
    model = build_stage1_iaai_model(cfg).to(device).eval()
    ...
    progress = tqdm(loader, total=total, desc="stage1 IAAI validation")
        ...
        for idx in range(batch_size):
            ...
            if args.save_limit is None or saved < int(args.save_limit):
                ...
                visual = make_stage1_gyro_visualization(
                    ...
                    title=f"IAAI aux gyro | {types[idx]} / {stems[idx]}",
