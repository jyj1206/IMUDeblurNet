...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Stage1 IAAI auxiliary gyro model.")
    parser.add_argument("--config", default="config/stage1_iaai_gyro.yaml")
...
def new_run_dir(config, resume=None):
    ...
    run_prefix = exp_cfg.get("run_prefix", "stage1_iaai_gyro")
...
def main():
    ...
    train_dataset, train_loader, train_sampler = build_stage1_iaai_loader(
        config,
    ...
    if config.get("validation", {}).get("enabled", True):
        _, val_loader, _ = build_stage1_iaai_loader(
            config,
    ...
    model = build_stage1_iaai_model(config).to(device)
